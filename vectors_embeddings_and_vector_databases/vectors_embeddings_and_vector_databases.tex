...
\title{Vectors, Embeddings, \& Vector Databases}
...
their own. More precisely, each dimension can end up carrying some
semantic signal, so the coordinates are not meaningless random values.
However, those meanings are not directly controlled by us or cleanly
labeled ahead of time. They emerge during training and are usually spread
across many dimensions in a way that is difficult to interpret exactly.
Instead, the full vector matters as a whole. The position of that vector
reflects the relationships the model learned during training. For a
deeper intuition, 3Blue1Brown has a short video on how directions or
...
This is the basic idea that leads directly to vector databases. A typical
vector database is built around storing many of these vector-backed
records, keeping them associated with IDs and metadata, and making
nearest-neighbor lookup fast enough to be practical even when the dataset
becomes very large. We will look at how vector databases usually do that
in the next section, but for now the important idea is simple: embeddings
become useful when they are placed inside a structure that supports
retrieval.
