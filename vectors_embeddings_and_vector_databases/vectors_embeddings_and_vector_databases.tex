...
\title{Retrieval, Vectors, Embeddings, \& Vector Databases}
...
same basic problem keeps showing up: given a query, return the text that
...
``RAV4.'' This works better than it sounds. Exact terms often matter a
lot. Model names, trims, years, drivetrain labels, engine codes, and VINs
usually need literal matching.
...
miss them for a boring reason: the words do not overlap enough.
...
term appears in this document. Inverse document frequency asks how rare
that term is across the whole collection. A useful search term is often
one that appears in this document but does not appear everywhere.
...
words matter. It is also how repeated matches should change the score.
...
electric car. Later mentions still help, but each extra mention tells us
less than the previous one. In other words, repetition should have
diminishing returns. A document that mentions ``electric'' five times is
probably more relevant than one that mentions it once. A document that
repeats ``electric'' 50 times is not 50 times better.
...
This is the gap semantic search is trying to fill. Instead of comparing
text only by word overlap, semantic search maps both the documents and the
query into vectors. Those vectors are arranged so that related meanings
tend to land near each other. The search problem changes from ``which
documents contain these words?'' to ``which stored meanings are closest
to this query?''
...
The simplest strategy is fixed-size chunking. For example, split every
document into chunks of 500 tokens, maybe with 50 tokens of overlap
between neighboring chunks. This is easy to implement and works
reasonably well when documents have consistent structure. The overlap is
there because important ideas often cross chunk boundaries. Without it, a
sentence at the end of one chunk and its explanation at the start of the
next chunk may get separated.

Another strategy is structure-aware chunking. Instead of splitting every
500 tokens, we split on headings, paragraphs, list items, sections, code
blocks, or other natural boundaries. This often produces chunks that feel
more coherent because the chunk follows the shape of the document rather
...
The basic rule is simple: chunking defines what the retriever can find. If
the chunks are too large, results may be vague. If they are too small,
results may be contextless. If they are split in awkward places, the
embedding may represent a fragment rather than a useful idea.
...
\subsection{Quantization In Practice}
